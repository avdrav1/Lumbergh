        ...
        try:
            ...
            if vision_success:
                thread = await thread_manager.create_bot_thread(
                    message=message,
                    thread_name=f"Analysis: {artwork['title'][:80]}",
                ...
                if not thread:
                    self.bot.logger.warning(
                        f"Artwork posted without analysis thread for {artwork['title']}"
                    )
            else:
...
Be insightful and educational. Around 300-400 words."""
            ...
            # Call Claude vision API
            message = await self.client.messages.create(
                model="claude-sonnet-4-5-20250929",  # Use Sonnet for better vision analysis
                max_tokens=800,
                messages=[{
                    "role": "user",
                    "content": [
