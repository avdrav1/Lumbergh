...
class Reminders(commands.Cog, name="reminders"):
    ...
    @commands.hybrid_command(name="remind", description="Set a reminder. Usage: /remind <time> <message> [recurring: True/False]")
    async def remind(self, context: Context, time: str, *, message: str, recurring: bool = False) -> None:
        """Set a reminder for a specified time."""
        print(f"📝 Reminder command called: {time} - {message} (Recurring: {recurring})")
        ...
        min_time = 10 if not recurring else 60
        if seconds < min_time:
            embed = discord.Embed(
                title="❌ Time Too Short",
                description=f"Please set a reminder for at least {min_time} seconds." + 
                           (" Recurring reminders need at least 1 minute." if recurring else ""),
                color=0xe74c3c
            )
            await context.send(embed=embed)
            return
            
        max_time = 31536000 if not recurring else 86400
        if seconds > max_time:
            embed = discord.Embed(
                title="❌ Time Too Long",
                description=f"Please set a reminder for less than {'1 year' if not recurring else '1 day'}.",
                color=0xe74c3c
            )
            await context.send(embed=embed)
            return
        ...
        recurring_text = f" every **{formatted_time}**" if recurring else f" in **{formatted_time}**"
        embed = discord.Embed(
            title="✅ Reminder Set",
            description=f"I'll remind you{recurring_text}:\n\n*{message}*",
            color=0x2ecc71,
            timestamp=datetime.utcnow()
        )
        if recurring:
        ...
        task = asyncio.create_task(
            self.reminder_worker(seconds, user_id, channel_id, message, formatted_time, recurring, seconds)
        )
    ...
    @commands.hybrid_command(name="recurring", description="Set a recurring reminder. Usage: /recurring <time> <message>")
    async def recurring_reminder(self, context: Context, time: str, *, message: str) -> None:
        """Set a recurring reminder for a specified time."""
        await self.remind(context, time, message=message, recurring=True)

    @commands.hybrid_command(name="schedule", description="Set a scheduled reminder. Usage: /schedule <pattern> <time> <message>")
    async def schedule_reminder(self, context: Context, pattern: str, time_str: str, *, message: str) -> None:
    ...
    async def list_reminders(self, context: Context) -> None:
        ...
        embed.set_footer(text="Use /stoprecurring, /stopscheduled, or /stopreminder to cancel reminders")
        await context.send(embed=embed)

    @commands.hybrid_command(name="stoprecurring", description="Stop all your recurring reminders")
    async def stop_recurring(self, context: Context) -> None:
    ...
    @commands.hybrid_command(name="stopscheduled", description="Stop all your scheduled reminders")
    async def stop_scheduled(self, context: Context) -> None:
    ...
    @commands.hybrid_command(name="stopreminder", description="Stop a specific reminder by message content")
    async def stop_reminder(self, context: Context, *, message_part: str) -> None:
    ...
    @commands.hybrid_command(name="testremin", description="Test reminder with 10 seconds (recurring optional)")
    async def test_reminder(self, context: Context, recurring: bool = False) -> None:
        """Test reminder with a 10 second delay."""
        if recurring:
            await self.recurring_reminder(context, "10s", message="This is a recurring test reminder!")
        else:
            await self.remind(context, "10s", message="This is a test reminder!")

    @commands.hybrid_command(name="testschedule", description="Test scheduled reminder (triggers in 1 minute from now)")
    async def test_scheduled_reminder(self, context: Context) -> None:
        """Test scheduled reminder with current time + 1 minute."""
        future_time = datetime.now() + timedelta(minutes=1)
        time_str = future_time.strftime('%H:%M')
        
        await self.schedule_reminder(context, "daily", time_str, message="This is a scheduled test reminder!")

    @commands.hybrid_command(name="reminstat", description="Show reminder system status")
    async def reminder_status(self, context: Context) -> None:
    ...
    @commands.hybrid_command(name="reminderhelp", description="Show help for reminder commands")
    async def reminder_help(self, context: Context) -> None:
        ...
        embed.add_field(
            name="🔄 Recurring Reminders",
            value="`/recurring <time> <message>` - Set a recurring reminder\n`/remind <time> <message> recurring:True` - Alternative syntax\n`/recurring 30m Stretch break` - Every 30 minutes\n`/recurring 1d Daily standup` - Every day",
            inline=False
        )

        embed.add_field(
            name="📅 Scheduled Reminders",
            value="`/schedule <pattern> <time> <message>` - Set scheduled reminder\n`/schedule daily 9:00am Morning coffee` - Every day at 9 AM\n`/schedule monday 2:30pm Weekly meeting` - Every Monday at 2:30 PM\n`/schedule weekdays 8am Wake up call` - Weekdays at 8 AM",
            inline=False
        )

        embed.add_field(
            name="📝 Managing Reminders",
            value="`/reminders` - List all your active reminders\n`/stoprecurring` - Stop all recurring reminders\n`/stopscheduled` - Stop all scheduled reminders\n`/stopreminder <text>` - Stop reminders containing text\n`/reminstat` - Show system status",
            inline=False
        )
        ...
        embed.add_field(
            name="🧪 Testing",
            value="`/testremin` - 10 second test\n`/testremin recurring:True` - Recurring test\n`/testschedule` - Scheduled test",
            inline=False
        )
