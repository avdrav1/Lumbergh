...
class TriviaView(discord.ui.View):
    ...
    async def handle_answer(self, interaction: discord.Interaction, answer: str):
        ...
        if is_correct:
            ...
            if current_streak > 1:
                response += f" | 🔥 {current_streak} streak!"
        else:
    ...
    async def on_timeout(self):
        ...
        if self.message:
            try:
                ...
                embed.set_footer(text=f"⏱️ Time's up! The correct answer was {self.correct_answer}")
...
- No questions about current events after 2024"""
    ...
    async def trivia(
        ...
        if not question_data:
            embed = discord.Embed(
                description="❌ Failed to generate trivia question. Please try again.",
                color=0xE02B2B
            )
    ...
    # ==================== COMMAND 2: /trivia-stats ====================
    @commands.hybrid_command(
        name="trivia-stats",
        description="View trivia statistics and leaderboards",
    )
    @app_commands.describe(
        action="What to display: leaderboard, personal, or categories",
        user="User to check stats for (only for 'personal' action)"
    )
    async def trivia_stats(
        ...
        """
        View trivia statistics and information.

        :param context: The command context.
        :param action: The action (leaderboard, personal, categories).
        :param user: User to check (for personal stats).
        """
        action = action.lower()

        if action == "leaderboard":
            await self._stats_leaderboard(context)
        elif action == "personal":
            await self._stats_personal(context, user)
        elif action == "categories":
            await self._stats_categories(context)
        else:
            embed = discord.Embed(
                description="❌ Invalid action. Use: `leaderboard`, `personal`, or `categories`.",
                color=0xE02B2B
            )
            await context.send(embed=embed)

    async def _stats_leaderboard(self, context: Context):
        ...
        try:
            ...
            for idx, (user_id, points, correct, answered, best_streak) in enumerate(results, 1):
                ...
                leaderboard_text.append(
                    ...
                    f"    💎 {points:,} pts | ✅ {accuracy:.1f}% | 🔥 {best_streak} streak"
                )
    ...
    async def _stats_personal(self, context: Context, user: Optional[discord.Member]):
        ...
        try:
            ...
            embed = discord.Embed(
                title=f"📊 Trivia Stats for {target_user.display_name}",
                color=0x9B59B6
            )
            ...
            try:
                ...
                if fav_result:
                    ...
                    emoji = self.CATEGORY_EMOJIS.get(fav_category, "❓")
                    embed.add_field(
    ...
    async def _stats_categories(self, context: Context):
    ...
    # ==================== COMMAND 3: /trivia-admin ====================
    @commands.hybrid_command(
        name="trivia-admin",
        description="Admin commands for trivia configuration (Admin only)",
    )
    @app_commands.describe(
        action="Admin action: schedule, toggle, or config",
        channel="Channel for scheduled trivia (schedule action)",
        time="Time in HH:MM format (schedule action)",
        timezone="UTC offset, e.g., -5 for EST (schedule action)",
        enabled="Enable or disable (toggle action)"
    )
    @commands.has_permissions(administrator=True)
    async def trivia_admin(
        self,
        context: Context,
        action: str,
        ...
        :param context: The command context.
        :param action: The action (schedule, toggle, config).
        :param channel: Channel for scheduled trivia.
        ...
        if action == "schedule":
            await self._admin_schedule(context, channel, time, timezone)
        elif action == "toggle":
            await self._admin_toggle(context, enabled)
        elif action == "config":
            await self._admin_config(context)
        else:
            embed = discord.Embed(
                description="❌ Invalid action. Use: `schedule`, `toggle`, or `config`.",
                color=0xE02B2B
            )
            await context.send(embed=embed)

    async def _admin_schedule(self, context: Context, channel: Optional[discord.TextChannel],
                              time: Optional[str], timezone: Optional[int]):
        """Schedule daily trivia games."""
        if not channel or not time or timezone is None:
            embed = discord.Embed(
                description="❌ Missing required parameters. Usage:\n`/trivia-admin schedule [channel] [time] [timezone]`",
                color=0xE02B2B
            )
            await context.send(embed=embed)
            return
        ...
        if not re.match(time_pattern, time):
            embed = discord.Embed(
                description="❌ Invalid time format. Use HH:MM (24-hour), e.g., 14:00",
                color=0xE02B2B
            )
            await context.send(embed=embed)
            return
        ...
        try:
            ...
            embed.set_footer(text="Use /trivia-admin toggle to disable")
            ...
            embed = discord.Embed(
                description="❌ Failed to schedule trivia. Please try again.",
                color=0xE02B2B
            )
            await context.send(embed=embed)

    async def _admin_toggle(self, context: Context, enabled: Optional[bool]):
        """Toggle scheduled trivia on/off."""
        if enabled is None:
            embed = discord.Embed(
                description="❌ Missing required parameter. Usage:\n`/trivia-admin toggle [enabled]`",
                color=0xE02B2B
            )
            await context.send(embed=embed)
            return
        ...
        try:
            ...
            if not result:
                embed = discord.Embed(
                    description="❌ Trivia is not configured yet. Use `/trivia-admin schedule` first.",
                    color=0xE02B2B
                )
                await context.send(embed=embed)
                return
            ...
            embed = discord.Embed(
                description=f"✅ Scheduled trivia has been **{status}**.",
            ...
            embed = discord.Embed(
                description="❌ Failed to toggle trivia. Please try again.",
                color=0xE02B2B
            )
            await context.send(embed=embed)

    async def _admin_config(self, context: Context):
        ...
        try:
            ...
            if not result:
                embed = discord.Embed(
                    ...
                    description="Trivia is not configured yet.\n\nUse `/trivia-admin schedule` to set up daily trivia games!",
                    color=0x3498DB
                )
                await context.send(embed=embed)
                return
            ...
            embed.set_footer(text="Use /trivia-admin to modify settings")
            ...
            embed = discord.Embed(
                description="❌ Failed to fetch configuration. Please try again.",
                color=0xE02B2B
            )
            await context.send(embed=embed)

    # ==================== COMMAND 4: /trivia-help ====================
    @commands.hybrid_command(
        name="trivia-help",
        description="Get help and tips for playing trivia",
    )
    async def trivia_help(self, context: Context) -> None:
        """
        Display help information for trivia commands.

        :param context: The command context.
        """
        embed = discord.Embed(
            title="❓ Trivia Help & Guide",
            description="Everything you need to know about trivia!",
            color=0x3498DB
        )
        ...
        embed.add_field(
            name="🎮 Commands",
            value=(
                "`/trivia [category] [difficulty]` - Play a trivia question\n"
                "`/trivia-stats leaderboard` - View server leaderboard\n"
                "`/trivia-stats personal [user]` - View personal stats\n"
                "`/trivia-stats categories` - List all categories\n"
                "`/trivia-admin schedule` - Setup scheduled trivia (Admin)\n"
                "`/trivia-admin toggle` - Enable/disable trivia (Admin)\n"
                "`/trivia-admin config` - View configuration (Admin)"
            ),
            inline=False
        )
        ...
        # Categories section
        categories_text = []
        for cat, desc in list(self.CATEGORIES.items())[:4]:
            emoji = self.CATEGORY_EMOJIS.get(cat, "❓")
            categories_text.append(f"{emoji} **{cat.title()}** - {desc}")

        embed.add_field(
            name="📚 Sample Categories",
            value="\n".join(categories_text) + "\n\n*Use `/trivia-stats categories` for the full list!*",
            inline=False
        )
        ...
        embed.add_field(
            ...
            value=(
                ...
                "**Streak Bonus:** +5 pts per streak (max +50)\n"
                "Answer correctly in a row to build your streak!"
            ),
            inline=False
        )
        ...
        embed.add_field(
            ...
            value=(
                ...
                "• Try harder difficulties for more points\n"
    ...
    async def post_scheduled_trivia(self, guild: discord.Guild, channel: discord.TextChannel,
        ...
        try:
            embed = discord.Embed(
                title="🎉 Daily Trivia Time!",
                description=f"It's time for today's trivia challenge!\n\nUse `/trivia` to play, or check out `/trivia-stats leaderboard` to see the leaderboard!",
                color=0x3498DB
            )
